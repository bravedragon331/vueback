...
var create = function(body, callback) {
  db.query(`
    INSERT INTO material (OrderIdx,A_Name,Supplier1,Order_Qty,Unit_Price,Amount1,Voucher_Date,Supplier2,Amount2) values (?,?,?,?,?,?,?,?,?)
...
var add = function(body, callback) {    
  db.query(`
    SELECT * FROM material WHERE OrderIdx = ? AND A_Name = ? AND Supplier1 = ? AND Order_Qty = ? AND
...
var load = function(ordidx, callback) {
  db.query('SELECT * FROM material WHERE OrderIdx = ?', [ordidx], function(err, list) {
    if(err) {
      return callback(err);
    } else {
...
var confirm = function(idx, data, callback) {
  console.log(data);
  db.query('UPDATE material SET ? WHERE Idx = ?', [{
...
var remove = function(body, callback) {
  db.query('DELETE FROM material WHERE Idx = ?', [body.Idx], function(err) {
    callback(err);
  })
}
