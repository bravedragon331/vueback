...
var create = function(body, callback) {
  db.query(`
    INSERT INTO fcplan (OrderIdx,F_Type,Fabric,U_Price,Consumption,Yds_Pcs,Width,Weight) values (?,?,?,?,?,?,?,?)
...
var add = function(body, callback) {
  db.query('SELECT * FROM fcplan WHERE OrderIdx = ? AND F_Type = ? AND Fabric = ? AND U_Price = ? AND Consumption = ? AND Yds_Pcs = ? AND Width = ? AND Weight = ?',
...
var load = function(ordidx, callback) {
  db.query('SELECT * FROM fcplan WHERE OrderIdx = ?', [ordidx], function(err, list) {
    if(err) {
      return callback(err);
    } else {
...
var confirm = function(idx, confirm, callback) {
  db.query('UPDATE fcplan SET ? WHERE Idx = ?', [{
    Confirm: confirm
  }, idx], function(err) {
    callback(err);
  })
...
var remove = function(body, callback) {
  db.query('DELETE FROM fcplan WHERE Idx = ?', [body.Idx], function(err) {
    callback(err);
  })
}
